...
pub const File = struct {
    ...
    pub fn inStream(file: File) io.InStream {
        return @implement(io.InStream, file, .{
            read = File.read,
        });
    }

    pub fn outStream(file: File) io.OutStream {
        return @implement(io.OutStream, file, .{
            .write = File.write,
        });
    }


    pub fn seekableStream(file: File) io.SeekableStream {
        return @implement(io.SeekableStream, file, .{
            .seekTo = File.seekTo,
            .seekBy = File.seekBy,
            .getPos = File.getPos,
            .getEndPos = File.getEndPos,
        });
    }
};
