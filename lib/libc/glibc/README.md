...
target defines which C library: `musl` is the statically linked
[musl C library](https://musl.libc.org/) or `gnu` which will be a
run-time, dynamic linking dependency on glibc.
...
later).  Use `zig env` to show the default targets and versions.
...
library that just needs to indicate that a given symbol will be present at
run-time, it doesn't actually have to implement said symbol).
